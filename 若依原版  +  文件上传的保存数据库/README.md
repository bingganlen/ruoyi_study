    **运行**
        jar包运行    java -jar RuoYi.jar或者是 ruoyi-admin.jar
        profiles:  active: druid   表示只运行application-druid.yml  测试环境  运行环境等弄不同的端口访问
        
        java -jar RuoYi.jar --spring.profiles.active=dev   运行jar文件 配置文件选择application-dev.yml 

    读取配置   所有Config结尾的类
       /   跳转到首页 视图映射
    ResourcesConfig  静态资源映射
       /profile/**   file:"F:/ruoyi/uploadPath/**"    映射硬盘本地    默认的是  spring.resources  即这几个路径  "classpath:/META-INF/resources/", "classpath:/resources/", "classpath:/static/", "classpath:/public/"
          
    缓存  
        调用位置  RuoYiApplication启动程序  @EnableCaching    
        对应的ServiceImpl   方法头上 @Cacheable(cacheNames = "config", key= "#configId")  名字加上key值
                                    public String selectConfigByKey(String configKey)    String configKey就是 上面的key= "#configId"
        注意：  修改的时候缓存也需要修改 @CachePut(cacheNames = "config", key= "#configId")   这个注解是存在的话就修改  不存在就新增到redis
        删除时  @CacheEvict(cacheNames = "config", allEntries = true) 与下面不对应 可以清空redis所有这个名字的
